...
class Avada {
	 ...
	 * RankMath module. Avada builder dequeues and deregisters native WP react scripts
	...
	public function add_json_data() {
		...
		if ( Helper::has_cap( 'onpage_snippet' ) ) {
			...
			// Trends.
			$trends_upgrade_link = 'https://rankmath.com/pricing/?utm_source=Plugin&utm_medium=Avada%20General%20Tab%20Trends&utm_campaign=WP';
	...
	public function footer_enqueue_scripts() {
		...
		wp_enqueue_style( 'rank-math-editor', rank_math()->plugin_url() . 'assets/admin/css/avada.css', [], rank_math()->version );

		wp_register_script( 'rank-math-analyzer', rank_math()->plugin_url() . 'assets/admin/js/analyzer.js', null, rank_math()->version, true );
		wp_enqueue_script( 'rank-math-editor', rank_math()->plugin_url() . 'assets/admin/js/avada.js', $avada_deps, rank_math()->version, true );
	...
	public function add_et_tag( $tag, $handle, $src ) {
		...
		if ( Str::starts_with( 'wp-', $handle ) || in_array( $handle, $script_handles, true ) ) {
			// These tags load in parent window only, not in Avada iframe.
	...
	public function print_react_containers() {
		echo '<div id="rank-math-rm-app-root"></div>';
	}


	/**
	 * Can add SEO in Avada Page Builder.
	 *
	 * @return bool
	 */
	private function can_add_seo_tab() {
		if (
			! Helper::is_avada_frontend_editor()
		) {
			return false;
		}

		/**
		 * Filter to show/hide SEO Tab in Avada Editor.
		 */
		if ( ! $this->do_filter( 'avada/add_seo_tab', true ) ) {
			return false;
		}
