...
class DataCollector {
	...
	getPostID() {
		return parseInt(
			FusionApp.data.postDetails.post_id || 0
		)
	}
	...
	getContentArea() {
		...
		const contentArea = jQuery( '#fb-preview' )
			.contents()
			.find( '#fusion_builder_container' )
	...
	async fetchFeaturedImageId() {
		...
		await apiFetch( {
			...
			method: 'POST',
			data: {
				postId: FusionApp.data.postDetails.post_id,
			},
	 ...
	 */
	subscribeToDivi() {
		this.setTitle( FusionApp.data.postDetails.post_title, '' )
		this.setExcerpt( FusionApp.data.postMeta.post_excerpt, '' )
		this.setFeaturedImage( FusionApp.data.postMeta._thumbnail_id, 0 )
		...
		jQuery( 'body' )
			.on( 'click', '.fusion-builder-save-page', () => {
